public int leastInterval(char[] tasks, int n) {
    ...
    int task[] = new int[26];               // Task array to store the count of each task,
                                            // initialised to 0.
    
    for(char i : tasks)  // For-each loop to iterate over tasks and update the task 
        task[i - 'A']++; // frequencies simultaneously.
    
    Arrays.sort(task);          // Sorting the frequencies in ascending order so
                                // that the highest frequencies come to last. 
    ...
    for(int i = 24; i >= 0; i--){           // Iterating over frequencies, except the
        ...
        idle = idle - Math.min(max_v, task[i]); // Updating the idle slots because as we do
                                                // some more tasks, the slots would be
                                                // filled and no. of idle slots will reduce.
    }                                       
    ...
    return tasks.length + idle; // we add them to total tasks.
}